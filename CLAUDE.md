...
Agent-first kanban board. React SPA + Hono API on a pure-local Node process with SQLite.
...
- Task lifecycle: Todo → Todo+assigned (local dispatch) → In Progress (agent claim) → In Review (agent review+PR) → Done (reviewer complete) or Cancelled (cancel at any stage). Reviewer = human or lead agent.
...
- SSE: long-lived connection with heartbeat + request-abort cleanup, Last-Event-ID resume via log ID → timestamp resolution (sse.ts). Emits typed events (`event: log` for task_logs, `event: message` for messages).
- Messages: `messages` table for human ↔ agent chat. `agent_id` = agent runtime session ID. SQLite as message bus — daemon/runtime sessions handle agent-side delivery, browser reads via SSE.
- Runtime implementation: **local daemon (`ak start`) is the source of truth** for runtime dispatch, quota/usage, health, and schedulability. Check `packages/cli/src/daemon/` and `apps/web/server/runtimeCoordinator.ts`.
...
## Post-Write Workflow
After every significant code change, follow the `ak-verify` skill (`skills/ak-verify/SKILL.md`): Tests → Review → Regression, with the Author/Test author/E2E author/Reviewer role split and ownership rule defined there.

Role mapping in this repository: Author = main agent; Test author = test-writer subagent; E2E author = playwright-test-generator (and playwright-test-healer for broken existing E2E tests) — invoked when changes touch frontend components (`apps/web/src/`); Reviewer = clean-code-reviewer subagent.
...
Project-specific additional step — **legacy daemon smoke test**: if changes explicitly touch deprecated daemon code (`packages/cli/src/daemon/`), run `./scripts/daemon-smoke-test.sh` and ensure it passes before considering that legacy path done.
...
- Unit tests: `*.test.ts` — direct import of modules, pure Node/jsdom, backed by better-sqlite3 or fake DBs.
- E2E tests: `*.spec.ts` — Playwright browser tests against the pure-local dev server (Node API + Vite proxy, local SQLite)
